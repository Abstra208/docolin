...
!!! info "A living document, and a young one"
    This policy describes how moderation on docolin is meant to work. The platform is early, so some of the mechanisms below are still being built and describe intent rather than what is live today. It will keep changing as the platform grows.
...
!!! warning "Urgent categories skip the queue"
    Some categories, illegal content, threats, doxxing, leaked secrets, and a few others, are routed directly to platform staff because they are time-sensitive or need platform-level judgment.

You will be notified through your docolin inbox when your report is resolved, with the outcome and any notes the reviewer left. If a report sits unresolved for a long time, or if dismissed content gets re-reported by other people, it is automatically escalated for platform staff to look at directly.

## What happens to your reported content

The actions we can take are deliberately few, and which one we use depends on why the content was reported.

### Hide

The content is marked invisible to other readers. You, the author, can still see what you wrote, with a notice explaining that it has been hidden and why. Doco moderators see that the content was hidden and the reason, but not the content itself. Only platform staff can see a hidden item's original body. This is deliberate: the fewer people with ongoing access to harmful content, the better for everyone.
...
In rare cases (a court order, a mandatory removal under law, a GDPR right-to-erasure request where no exception applies), we may delete content entirely. This is unusual and only done with platform staff approval.
...
- Reach out to platform staff if you believe the action was wrong, or if you want to address the concern and have the hide reversed.
...
!!! info "Embargoes lift themselves"
    If your content was hidden under an embargo (for example, a security vulnerability disclosed before a patch ships), the hide carries a target date. When it passes, your content becomes visible again automatically and you are notified.
...
Reporters who file many reports that are dismissed may have their reports deprioritized. Reporters who act in bad faith (mass flagging, retaliatory reporting, coordinated harassment through the report system) may be restricted from reporting at all. We will tell you if this happens to you.

## Who can do what

### Doco moderators

Each doco has moderators, typically the publisher and anyone they delegate to. They **can**:

- Lock a discussion (no new replies; existing content stays visible).
...
- File a deletion request, which immediately hides the content pending platform-staff review.

They **cannot**:

- Hide, redact, or remove content outright on their own. A deletion request only hides content pending review; platform staff make the final call.
- See content once it has been hidden (only platform staff can).
- Override a decision made by platform staff.

The intent is to give moderators real authority over their own communities while keeping ongoing access to potentially harmful content as narrow as possible.
...
Every moderation action, by doco moderators and platform staff alike, is logged.
...
If you believe an action against your content was wrong, reach out to `support@docolin.com`. Platform staff will review the case. If a doco moderator made the original call, the review is done by someone other than that moderator.
