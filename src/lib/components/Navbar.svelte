<script lang="ts">
  ...
  // Hard navigation (not SvelteKit's goto) because the reroute hook collapses
  // /de/foo and /foo onto the same route, so SvelteKit sees no route change
  // and skips re-rendering. A full page load forces SSR with the new locale.
    ...
    <nav class="grid grid-cols-[1fr_auto_1fr] items-center gap-4 px-4 py-2.5 sm:px-6">
      ...
      <div class="flex items-center gap-1.5 justify-self-end">
        <Select.Root type="single" value={getLocale()} onValueChange={changeLocale}>
          <Select.Trigger class="hidden h-9 w-auto sm:flex" aria-label={m.nav_lang_aria()}>
