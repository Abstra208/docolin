<script lang="ts">
  ...
  // The taxonomy's top-level kinds as a single alphabetical card grid. The root
  // list is static; counts and descriptions come from the hard-cached load.
  // Each card links to that root's kind browse page, which handles its own
  // empty state for roots with no docos.
