<script lang="ts">
  ...
  async function loadDone(): Promise<void> {
    loadError = null;
    try {
      const res = await fetch("/api/dashboard/inbox?bucket=done", {
  ...
  // Drop a row from the client-fetched list when its move-back succeeds.
...
{#if loadError !== null}
  ...
  >
    ...
    <Button type="button" variant="outline" size="sm" onclick={() => void loadDone()}>
